Given /I am an authenticated user/ do
  email = 'foo@gmail.com'
  password = '12345678'
  @user = User.create({:email => email, :password => password, :first_name => 'foo', :last_name => 'bar'})
...
When /I request "(.+)" on (\d\d-\d\d-\d\d\d\d) from (\d\d:\d\d [AP]M) to (\d\d:\d\d [AP]M)/ do |location, day, start_time, end_time|
  ...
  select(date_vals[1].to_i.to_s, :from => 'space_rental_request_start_3i')
  ...
  select(date_vals[1].to_i.to_s, :from => 'space_rental_request_end_3i')
...
Given /"(.+)" is reserved on (\d\d-\d\d-\d\d\d\d) from (\d\d:\d\d [AP]M) to (\d\d:\d\d [AP]M)/ do |location, day, start_time, end_time|
  ...
  request = SpaceRentalRequest.create(:location => location, :start => start_datetime, :end => end_datetime, :approved => true)
  request.user = @user
end
